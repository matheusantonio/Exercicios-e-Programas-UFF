...
void menorCusto(int *vet, Grafo **g, int pos, int destino, int **vvets, float *menor, int *posMenor)
{
    if(vet[pos-1]==destino)
    {
        float custoAtual = somaCusto(vet, g, pos);
        if(custoAtual < *menor)
        {
            *menor = custoAtual;
    ...
    else
    {
        ...
        while(l!=NULL)
        {
            ...
            {
                vet[pos] = l->destino;
                menorCusto(vet, g, pos+1, destino, vvets, menor, posMenor);
            }
            l = l->prox;
        }
    }
